...
class DailymotionExtractor(private val client: OkHttpClient, private val headers: Headers) {
    ...
    companion object {
        ...
        private val INTERNAL_DATA_REGEX by lazy { Regex("""(?i)"dmInternalData"\s*:\s*(\{.+?\}),""") }
        private val DMVK_REGEX by lazy { Regex("""(?i)"dmvk"\s*:\s*"(.+?)"""") }
        private val CLIENT_ID_REGEX by lazy { Regex("""(?i)"client_id"\s*:\s*"(.+?)"""") }
        private val CLIENT_SECRET_REGEX by lazy { Regex("""(?i)"client_secret"\s*:\s*"(.+?)"""") }
    }
    ...
    fun videosFromUrl(url: String, headers: Headers, prefix: String = "Dailymotion - ", passwordProvider: () -> String? = { null }): List<Video> = videosFromUrl(url, prefix, passwordProvider)

    fun videosFromUrl(url: String, prefix: String = "Dailymotion - ", passwordProvider: () -> String? = { null }): List<Video> {
        val htmlString = client.newCall(GET(url, headers)).execute().body.string()

        val internalData = INTERNAL_DATA_REGEX.find(htmlString)?.groupValues?.get(1)
            ?: return emptyList()
        ...
        return when {
            ...
            parsed.error?.type == "password_protected" && parsed.id != null -> {
                videosFromProtectedUrl(url, prefix, parsed.id, htmlString, ts, v1st, passwordProvider)
            }
            else -> emptyList()
        }
    }
    ...
    private fun videosFromProtectedUrl(
        ...
        val clientId = CLIENT_ID_REGEX.find(htmlString)?.groupValues?.get(1) ?: return emptyList()
        val clientSecret = CLIENT_SECRET_REGEX.find(htmlString)?.groupValues?.get(1) ?: return emptyList()
        val scope = htmlString.substringAfter("client_scope\":\"").substringBefore('"')
        val password = passwordProvider() ?: return emptyList()
        ...
        val dmvk = DMVK_REGEX.find(htmlString)?.groupValues?.get(1) ?: return emptyList()
        val getVideoIdUrl = "$DAILYMOTION_URL/player/metadata/video/${idParsed.xid}?locale=en-US&dmV1st=$v1st&dmTs=$ts&is_native_app=0"
