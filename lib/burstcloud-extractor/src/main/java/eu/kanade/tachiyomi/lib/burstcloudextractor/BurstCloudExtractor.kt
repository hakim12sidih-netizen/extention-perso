...
class BurstCloudExtractor(private val client: OkHttpClient, private val headers: Headers) {

    private val json: Json by injectLazy()

    fun videoFromUrl(url: String, headers: Headers, name: String = "BurstCloud"): List<Video> = videoFromUrl(url, headers, name, "")
    ...
    fun videoFromUrl(url: String, headers: Headers, name: String = "BurstCloud", prefix: String = ""): List<Video> {
        ...
        return runCatching {
            ...
            if (videoUrl.isNotEmpty()) {
                val quality = name.let { if (prefix.isNotBlank()) "$prefix $it" else it }
    ...
    private data class BurstCloudDto(
