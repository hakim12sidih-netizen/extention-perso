...
class StreamTapeExtractor(private val client: OkHttpClient, private val headers: Headers) {
    fun videoFromUrl(url: String, quality: String = "StreamTape", subtitleList: List<Track> = emptyList()): Video? {
        ...
        val document = client.newCall(GET(newUrl, headers)).execute().asJsoup()
    ...
    fun videosFromUrl(url: String, headers: Headers, subtitleList: List<Track> = emptyList()): List<Video> = videosFromUrl(url, "StreamTape", subtitleList)
