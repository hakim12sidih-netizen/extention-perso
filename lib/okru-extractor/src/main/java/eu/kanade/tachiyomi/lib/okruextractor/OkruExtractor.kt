...
class OkruExtractor(private val client: OkHttpClient, private val headers: Headers) {
    ...
    fun videosFromUrl(url: String, headers: Headers, fixQualities: Boolean = true): List<Video> = videosFromUrl(url, "Okru", fixQualities)
    ...
    fun videosFromUrl(url: String, prefix: String = "", fixQualities: Boolean = true): List<Video> {
        ...
        return when {
            "ondemandHls" in videoString -> {
                ...
                playlistUtils.extractFromHls(playlistUrl, videoNameGen = { "Okru:$it".addPrefix(
                    prefix
                ) })
            }
            "ondemandDash" in videoString -> {
                val playlistUrl = videoString.extractLink("ondemandDash")
                playlistUtils.extractFromDash(playlistUrl, videoNameGen = { it -> "Okru:$it".addPrefix(
                    prefix
                ) })
            }
