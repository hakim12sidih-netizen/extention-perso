...
class AnimeSama : AnimeDomainSource() {
    ...
    override suspend fun getVideoList(episode: SEpisode): List<Video> {
        ...
        val videos = playerUrls.flatMapIndexed { i, it ->
            ...
            it.parallelCatchingFlatMap { playerUrl ->
                with(playerUrl) {
                    when {
                        contains("sibnet.ru") -> sibnetExtractor.videosFromUrl(playerUrl, headers)
                        contains("vk.") -> vkExtractor.videosFromUrl(playerUrl, headers)
                        contains("sendvid.com") -> sendvidExtractor.videosFromUrl(playerUrl, headers)
                        contains("vidmoly.to") -> vidMolyExtractor.videosFromUrl(playerUrl, headers)
                        else -> emptyList()
                    }
                }
            }
