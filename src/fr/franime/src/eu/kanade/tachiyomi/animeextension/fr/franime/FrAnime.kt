...
class FrAnime : AnimeHttpSource() {
    ...
    private val database by lazy {
        val cookieHeader = client.cookieJar.loadForRequest(baseUrl.toHttpUrl())
            .joinToString("; ") { "${it.name}=${it.value}" }
        val newHeaders = headers.newBuilder()
            .add("Cookie", cookieHeader)
            .build()
        client.newCall(GET("$baseApiUrl/animes/", newHeaders)).execute()
            .body.string()
    ...
    override suspend fun getVideoList(episode: SEpisode): List<Video> {
        ...
        val sibnetExtractor by lazy { SibnetExtractor(client, headers) }
        ...
        val videos = players.withIndex().parallelCatchingFlatMap { (index, playerName) ->
            ...
            val playerUrl = client.newCall(GET(apiUrl, newHeaders)).await().body.string().trim('"').replace("\\/", "/")
            when (playerName) {
