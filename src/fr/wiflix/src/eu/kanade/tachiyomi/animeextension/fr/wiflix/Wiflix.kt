...
class Wiflix : DataLifeEngine(
    ...
    override fun episodeFromElement(element: Element): SEpisode = SEpisode.create().apply {
        ...
        url = element.select("a").joinToString(",") { it.attr("onclick").substringAfter("'").substringBefore("'") }
    }
    ...
    override suspend fun getVideoList(episode: SEpisode): List<Video> {
        val list = episode.url.split(",").filter { it.isNotBlank() }.parallelCatchingFlatMap {
            with(it) {
                when {
                    contains("doods.pro") ||
                        contains("do7go.com") -> DoodExtractor(client, headers).videosFromUrl(this)
                    contains("vido.lol") -> VidoExtractor(client, headers).videosFromUrl(this)
                    contains("uqload.co") -> UqloadExtractor(client, headers).videosFromUrl(this)
                    contains("waaw1.tv") -> emptyList()
                    contains("vudeo.co") -> VudeoExtractor(client, headers).videosFromUrl(this)
                    contains("streamvid.net") -> StreamHideVidExtractor(client, headers).videosFromUrl(this)
                    contains("upstream.to") -> UpstreamExtractor(client, headers).videosFromUrl(this)
                    contains("streamdav.com") -> StreamDavExtractor(client, headers).videosFromUrl(this)
                    contains("voe.sx") ||
                        contains("magasavor.net") -> VoeExtractor(client, headers).videosFromUrl(this)
                    else -> emptyList()
                }
            }
        }.sort()
